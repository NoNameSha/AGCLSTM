...
LSTM_IN_DIM = 22    # TX11，CH7，HH9 #node Lstm_in_dim(Chev_out*node)
#IN_DIM = int(num_timesteps_input*LSTM_IN_DIM)
# LSTM_HIDDEN_DIM = num_timesteps_input*LSTM_IN_DIM*2  # LSTM隐状态的大小
LSTM_HIDDEN_DIM = 264  #（34*12 lstm_in_dim * time_steps）
...
def test(dataloader):
    ...
    if dataset_name == 'TX':
        ...
        groundtruth = np.array(groundtruth)
        np.savetxt(dataset_name+"_groundtruth.csv", groundtruth[:,6,:]*stds[1]+means[1], delimiter=",")    
    # if dataset_name == 'TX':
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        loss_val = validate(val_dataloader)
        loss_test = test(test_dataloader)

        training_losses.append(loss)
        adjust_lr.step()

        print('epoch = %d,loss = %.5f,loss_val=%.5f,loss_test=%.5f' % (epoch+1, loss, loss_val, loss_test), time.asctime( time.localtime(time.time()) ))
